...
public class JdbcParquetSchema {
  ...
  private static Logger LOGGER = LoggerFactory.getLogger(JdbcParquetSchema.class);
  ...
  public static Schema createSchemaByReadingOneRow(
    ...
    try (Statement statement = connection.createStatement()) {
      ...
      Schema schema = JdbcParquetSchema.createAvroSchema(
  ...
  private static SchemaBuilder.FieldAssembler<Schema> fieldAvroType(
    ...
    switch (columnType) {
      ...
      case BIGINT:
        if (precision > 0 && precision <= JdbcParquetRecord.MAX_DIGITS_BIGINT) {
